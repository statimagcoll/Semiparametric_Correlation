...
plot_SE_SD_panel <- function(dt_suffixes, dt_labels, ...) {
  ...
  plots_nolegend <- map2(plots, seq_along(plots), function(p, i) {
    ...
    p + theme(
      ...
      axis.title.y = if (!is_left_col) element_blank() else element_text(size = 22),
...
p = plot_SE_SD_panel(c("ridge_age", "rf_age"), c("Ridge", "Random Forest"), lims = c(0, 0.13))

p

# save for journal submission
ggsave("FigS2.pdf", p, width = 14, height = 7, device = cairo_pdf)
```
