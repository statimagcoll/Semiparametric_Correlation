---
title: "RBC Random Forest-Random Forest Simulations (Functional)"
author: "Megan Jones"
date: "`r Sys.Date()`"
output:
  html_document: 
...
This file performs the simulations used for main text Figure 3.

In this simulation, $\xi$ is equal to $\mu$, leading to 3 parameters.

$\xi$ and $\mu$ are tuned random forest models.
...
perform_sim = function(pheno, rho_true, seed_ind, ncores, results_path, ...){
 ...
 eval_train$fakeYMean = predict(rfTune$model, newdata = data.frame(eval_train$img))$data$response
  ...
  muhat_tr_mod = ranger(x = eval_train$img, y = eval_train$fakeY, num.threads = 1)
...
```{r, eval=T}
seed_ind = 2
results_path_root = "/media/disk2/multivariateBWAS/RBC_sims/FC_sims/results/rf_17network/"
...
```{r, eval=F}
knitr::purl("/media/disk2/multivariateBWAS/RBC_sims/FC_sims/FC_rfximu.Rmd")
```
