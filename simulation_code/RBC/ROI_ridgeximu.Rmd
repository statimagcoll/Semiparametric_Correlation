---
title: "RBC Ridge-Ridge Simulations (Structural)"
author: "Megan Jones"
date: "`r Sys.Date()`"
output:
  html_document: 
...
These simulations are for Figure 2, Figure S2, and Figure S5.
...
While we compute all the transformed estimators and the different bootstrapping methods, we will focus on PQ inst and the percentile-t bootstrap as the comparison to Pearson inst and the Fisher confidence interval
...
```{r, eval=T}
seed_ind = 2
results_path_root = "/media/disk2/multivariateBWAS/RBC_sims/ROI_sims/results/ridge_build_cov_unif_schaefer/"
# phenotypes to predict
#phenotypes = c("age", "internal", "external")
phenotypes = c("p_factor")
...
build$p_factor = scale(build$p_factor) # haven't run the scaled p_factor yet
...
```{r}
#ns <- c(100, 200, 500, 750, 1000, 1500, 2000, 3000, 4000, 5000, 6000, 7500, 10000)
...
```{r, eval=F}
knitr::purl("/media/disk2/multivariateBWAS/RBC_sims/ROI_sims/ROI_ridgeximu.Rmd")
```
