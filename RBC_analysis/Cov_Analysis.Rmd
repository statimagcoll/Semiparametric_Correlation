...
get_OS_diff <- function(OS_struct, OS_func, OS_both, influ_struct, influ_func, influ_both, outcome){
  # Calculating differences
  diff_struct <- cor_os_diff(OS_struct, OS_both, influ_struct, influ_both)
  diff_func <- cor_os_diff(OS_func, OS_both, influ_func, influ_both)
  
  # Converting differences to data frame
  diff_struct = data.frame(diff = diff_struct[1], lCI_diff = diff_struct[2], uCI_diff = diff_struct[3], outcome = outcome, Type = "Structural vs. Both")
  diff_func = data.frame(diff = diff_func[1], lCI_diff = diff_func[2], uCI_diff = diff_func[3], outcome = outcome, Type = "Functional vs. Both")
  
  # Combining all data together
  diff_all <- rbind(diff_struct, diff_func)
...
# Getting the bounds and OS difference for an outcome variable 
# Outcome is the string name of the predicted variable
get_OS_ratio <- function(OS_struct, OS_func, OS_both, influ_struct, influ_func, influ_both, outcome){
  # Calculating differences
  ratio_struct <- cor_os_ratio(OS_struct, OS_both, influ_struct, influ_both)
  ratio_func <- cor_os_ratio(OS_func, OS_both, influ_func, influ_both)
  
  # Converting differences to data frame
  ratio_struct = data.frame(ratio = ratio_struct[1], lCI_ratio = ratio_struct[2], uCI_ratio = ratio_struct[3], outcome = outcome, Type = "Structural vs. Both")
  ratio_func = data.frame(ratio = ratio_func[1], lCI_ratio = ratio_func[2], uCI_ratio = ratio_func[3], outcome = outcome, Type = "Functional vs. Both")
  
  # Combining all data together
  ratio_all <- rbind(ratio_struct, ratio_func)
...
RidgeCV <- function(data, folds, outcome, typeDat){
  ...
  for(i in 1:folds){
    ...
    # Assigning x and y 
    if(outcome == "age"){
      x_var = data.matrix(train[, colnames(train) %in% c("sex", "euler")]) # can add euler here
      y_var = data.matrix(train[, outcome])
      newx = data.matrix(test[, colnames(test) %in% c("sex", "euler")])
    }
    
    else{
      x_var = data.matrix(train[, colnames(train) %in% c("sex", "age", "euler")])
      y_var = data.matrix(train[, outcome])
      newx = data.matrix(test[, colnames(test) %in% c("sex", "age", "euler")])
    }
...
getAnalysis <- function(data, outcome, struct_string, func_string, folds, CI, label){
  ...
  results <- rbind(struct_results, func_results, both_results)
  return(results)
}
...
getSampleSplit <- function(data, outcome, struct_string, func_string, folds, splits, CI, label){
  ...
  while(splits > 0){ # Add split number here 
    ...
    diff <- get_OS_diff(temp$estimates[2], temp$estimates[4], temp$estimates[6], temp$Inf_Vec[[2]], temp$Inf_Vec[[4]], temp$Inf_Vec[[6]], outcome)
    ratio <- get_OS_ratio(temp$estimates[2], temp$estimates[4], temp$estimates[6], temp$Inf_Vec[[2]], temp$Inf_Vec[[4]], temp$Inf_Vec[[6]], outcome)
...
results$label <- factor(results$label,
                        levels = c("Structural Data", "Functional Data", "Both"))
...
corr_results <- ggplot(results, aes(x = label, y = estimates, color = stat)) +
  ...
  labs(title = "Ridge Regression Analysis of Outcomes in RBC Only Covariates",
       x = "Model Features",
       y = "Correlation",
       color = "Estimator") +
  theme_bw() + 
...
ggsave("/media/disk2/multivariateBWAS/ishaan_bwas/Cov_RR_results_100.png",
       plot = corr_results,
       width = 9,  # make wider
...
saveRDS(corr_results, file = "/media/disk2/multivariateBWAS/ishaan_bwas/Cov_RR_results_100.rds")
```
...
results$Type <- factor(results$Type,
                        levels = c("Structural vs. Both", "Functional vs. Both"))
...
diff_results <- ggplot(results, aes(x = Type, y = diff)) +
  ...
  geom_errorbar(aes(ymin = lCI_diff, ymax = uCI_diff),
                ...
                position = position_dodge(width = 0.5)) +
  facet_wrap(~ ScoreType, nrow = 1) +  # or facet_grid(~ ScoreType)
  labs(title = "Ridge Regression One-Step Differences in RBC Only Covariates",
       x = "Comparisons",
       y = "Difference") +
  theme_bw() + 
  theme(axis.text.x = element_text(angle = 45, hjust=1)) + 
...
# Saving to square dimensions
ggsave("/media/disk2/multivariateBWAS/ishaan_bwas/Cov_RR_results_diff_100.png",
       plot = diff_results,
       width = 9,  # make wider
       height = 4,  # adjust height
       dpi = 300)
...
# Saving ggplot object itself
saveRDS(diff_results, file = "/media/disk2/multivariateBWAS/ishaan_bwas/Cov_RR_results_diff_100.rds")
```
...
ratio_results <- ggplot(results, aes(x = Type, y = ratio)) +
  ...
  geom_errorbar(aes(ymin = lCI_ratio, ymax = uCI_ratio),
                ...
                position = position_dodge(width = 0.5)) +
  facet_wrap(~ ScoreType, nrow = 1) +  # or facet_grid(~ ScoreType)
  labs(title = "Ridge Regression One-Step Ratios in RBC Only Covariates",
       x = "Comparisons",
       y = "Ratio") +
  theme_bw() + 
  theme(axis.text.x = element_text(angle = 45, hjust=1)) + 
...
# Saving to square dimensions
ggsave("/media/disk2/multivariateBWAS/ishaan_bwas/Cov_RR_results_ratio_100.png",
       plot = ratio_results,
       width = 9,  # make wider
       height = 4,  # adjust height
       dpi = 300)
...
# Saving ggplot object itself
saveRDS(ratio_results, file = "/media/disk2/multivariateBWAS/ishaan_bwas/Cov_RR_results_ratio_100.rds")
```
...
# Saving tables
save_kable(ext_kable, "/media/disk2/multivariateBWAS/ishaan_bwas/Cov_RR_Externalizing_Table_100.html")
save_kable(int_kable, "/media/disk2/multivariateBWAS/ishaan_bwas/Cov_RR_Internalizing_Table_100.html")
save_kable(age_kable, "/media/disk2/multivariateBWAS/ishaan_bwas/Cov_RR_Age_Table_100.html")
save_kable(pfac_kable, "/media/disk2/multivariateBWAS/ishaan_bwas/Cov_RR_PFactor_Table_100.html")
```
...
# Saving tables
save_kable(ext_kable, "/media/disk2/multivariateBWAS/ishaan_bwas/Cov_RR_Externalizing_DR_Table_100.html")
save_kable(int_kable, "/media/disk2/multivariateBWAS/ishaan_bwas/Cov_RR_Internalizing_DR_Table_100.html")
save_kable(age_kable, "/media/disk2/multivariateBWAS/ishaan_bwas/Cov_RR_Age_DR_Table_100.html")
save_kable(pfac_kable, "/media/disk2/multivariateBWAS/ishaan_bwas/Cov_RR_PFactor_DR_Table_100.html")
```
