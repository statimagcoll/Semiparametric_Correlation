...
This performs the ridge regression analysis for the full RBC dataset (Figures 4, 5, S6).
...
makeGraph <- function(graph1, graph2){
  ...
  shared_legend <- get_legend(graph1 + theme(legend.position = "right", 
  ...
  final_fig <- plot_grid(
    ...
    shared_legend,
    ncol = 2,
    rel_widths = c(1, 0.175)  # Increase 0.15 → 0.175 to move legend farther right
  )
...
RBCTot100 <- readRDS("RBC_RR_results_100.rds")
RFTot100 <- readRDS("RBC_RF_results_100.rds")
...
final_fig <- makeGraph(RFTot100, RBCTot100)
final_fig
...
```{r}
# Reading in graphs for total analysis
RBCTot100_Diff <- readRDS("RBC_RR_results_diff_100.rds")
RFTot100_Diff <- readRDS("RBC_RF_results_diff_100.rds")
...
# Running the function
final_fig <- makeGraph(RFTot100_Diff, RBCTot100_Diff)
final_fig
...
```{r}
# Reading in graphs for total analysis
RBCTot100_Ratio <- readRDS("RBC_RR_results_ratio_100.rds")
RFTot100_Ratio <- readRDS("RBC_RF_results_ratio_100.rds")
...
# Running the function
final_fig <- makeGraph(RFTot100_Ratio, RBCTot100_Ratio)
final_fig
...
```{r}
# Reading in graphs for total analysis
RRHBN250 <- readRDS("HBN_RR_results_250.rds")
RFHBN250 <- readRDS("HBN_RF_results_250.rds")
...
# Running the function
final_fig <- makeGraph(RFHBN250, RRHBN250)
final_fig
...
```{r}
# Reading in graphs for total analysis
RRHBN250_diff <- readRDS("HBN_RR_results_diff_250.rds")
RFHBN250_diff <- readRDS("HBN_RF_results_diff_250.rds")

# Setting y limits
RRHBN250_diff <- RRHBN250_diff + ylim(-2, 2.0)
RFHBN250_diff <- RFHBN250_diff + ylim(-2, 2.0)

# Running the function
final_fig <- makeGraph(RFHBN250_diff, RRHBN250_diff)
final_fig
...
```{r}
# Reading in graphs for total analysis
RRHBN250_ratio <- readRDS("HBN_RR_results_ratio_250.rds")
RFHBN250_ratio <- readRDS("HBN_RF_results_ratio_250.rds")
...
# Running the function
final_fig <- makeGraph(RFHBN250_ratio, RRHBN250_ratio)
final_fig
