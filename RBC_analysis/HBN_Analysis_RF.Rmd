---
title: "HBN Site SI Analysis Random Forest"
...
This performs the random forest analysis for HBN SI site (Figures S7, S8, Table S3).
...
both_total <- dplyr::inner_join(rbc, func[, c("participant_id", cols_to_add)], by = "participant_id")

# Subsetting study site to HBNsiteSI
both = both_total %>% filter(study_site == "HBNsiteSI")
...
label(both_total$age) <- "Age (Years)"
label(both_total$sex) <- "Sex"
label(both_total$internalizing_mcelroy_harmonized_all_samples) <- "Internalizing Score"
label(both_total$externalizing_mcelroy_harmonized_all_samples) <- "Externalizing Score"
label(both_total$p_factor_mcelroy_harmonized_all_samples) <- "P-Factor"
label(both_total$meanFD) <- "Mean Framewise Displacement"
label(both_total$euler) <- "Euler Number"
...
# Creating demographic table for HBNsiteSI
tblHBN <- table1(~ age + sex + internalizing_mcelroy_harmonized_all_samples + externalizing_mcelroy_harmonized_all_samples + 
         p_factor_mcelroy_harmonized_all_samples + meanFD + euler,
       data= both,
       caption="HBN Site SI Demographics")

tblHBN_df <- as.data.frame(tblHBN)
write.csv(tblHBN_df, "HBN_Dems.csv", row.names = FALSE)
...
corr_results <- ggplot(results, aes(x = label, y = estimates, color = stat)) +
  ...
  labs(title = "Random Forest Analysis of Outcomes in HBN Site SI",
       x = "Model Features",
       y = "Correlation",
       color = "Estimator") +
  theme_bw() + 
...
ggsave("/media/disk2/multivariateBWAS/ishaan_bwas/HBN_RF_results_250.png",
       plot = corr_results,
       width = 9,  # make wider
...
saveRDS(corr_results, file = "/media/disk2/multivariateBWAS/ishaan_bwas/HBN_RF_results_250.rds")
```
...
diff_results <- ggplot(results, aes(x = Type, y = diff)) +
  ...
  geom_errorbar(aes(ymin = lCI_diff, ymax = uCI_diff),
                ...
                position = position_dodge(width = 0.5)) +
  facet_wrap(~ ScoreType, nrow = 1) +  # or facet_grid(~ ScoreType)
  labs(title = "Random Forest One-Step Differences in HBN Site SI",
       x = "Comparisons",
       y = "Difference") +
  theme_bw() + 
  theme(axis.text.x = element_text(angle = 45, hjust=1)) + 
...
# Saving to square dimensions
ggsave("/media/disk2/multivariateBWAS/ishaan_bwas/HBN_RF_results_diff_250.png",
       plot = diff_results,
       width = 9,  # make wider
       height = 4,  # adjust height
       dpi = 300)
...
# Saving ggplot object itself
saveRDS(diff_results, file = "/media/disk2/multivariateBWAS/ishaan_bwas/HBN_RF_results_diff_250.rds")
```
...
ratio_results <- ggplot(results, aes(x = Type, y = ratio)) +
  ...
  geom_errorbar(aes(ymin = lCI_ratio, ymax = uCI_ratio),
                ...
                position = position_dodge(width = 0.5)) +
  facet_wrap(~ ScoreType, nrow = 1) +  # or facet_grid(~ ScoreType)
  labs(title = "Random Forest One-Step Ratios in HBN Site SI",
       x = "Comparisons",
       y = "Ratio") +
  theme_bw() + 
  theme(axis.text.x = element_text(angle = 45, hjust=1)) + 
...
# Saving to square dimensions
ggsave("/media/disk2/multivariateBWAS/ishaan_bwas/HBN_RF_results_ratio_250.png",
       plot = ratio_results,
       width = 9,  # make wider
       height = 4,  # adjust height
       dpi = 300)
...
# Saving ggplot object itself
saveRDS(ratio_results, file = "/media/disk2/multivariateBWAS/ishaan_bwas/HBN_RF_results_ratio_250.rds")
```
...
# Saving tables
save_kable(ext_kable, "/media/disk2/multivariateBWAS/ishaan_bwas/HBN_RF_Externalizing_Table_250.html")
save_kable(int_kable, "/media/disk2/multivariateBWAS/ishaan_bwas/HBN_RF_Internalizing_Table_250.html")
save_kable(age_kable, "/media/disk2/multivariateBWAS/ishaan_bwas/HBN_RF_Age_Table_250.html")
save_kable(pfac_kable, "/media/disk2/multivariateBWAS/ishaan_bwas/HBN_RF_PFactor_Table_250.html")
```
...
# Saving tables
save_kable(ext_kable, "/media/disk2/multivariateBWAS/ishaan_bwas/HBN_RF_Externalizing_DR_Table_250.html")
save_kable(int_kable, "/media/disk2/multivariateBWAS/ishaan_bwas/HBN_RF_Internalizing_DR_Table_250.html")
save_kable(age_kable, "/media/disk2/multivariateBWAS/ishaan_bwas/HBN_RF_Age_DR_Table_250.html")
save_kable(pfac_kable, "/media/disk2/multivariateBWAS/ishaan_bwas/HBN_RF_PFactor_DR_Table_250.html")
```
